...
import threading
import random
from flask import Flask, render_template, Response, request, send_from_directory, make_response, jsonify
global ServerKey
ServerKey = "Shaul"
...
def home():
    ...
    ServerKey = random.randint(0, 100)
    ...
    if mode == "start":
        ...
        run = False
    response = make_response(jsonify({"key": ServerKey}))
...
class Nema:
    ...
    def do_step(self, direction, amount=0):
        ...
        time.sleep(0.0005 if x <= xCenter <= x + w and y <= yCenter <= y + h else 0.0005)
        GPIO.output(self.step, GPIO.LOW)
        time.sleep(0.0025)
    @staticmethod
    def reset():
...
def checkCenteredObject():
    l, r = mid_x
    t, b = mid_y
    if l <= xCenter <= r and t <= yCenter <= b:
        return True
    return False
def move_motors():
    while not on_target:
...
def main():
    ...
    while True:
        ...
        if run:
            ...
            greenPin.on()
            redPin.off()
        cv2.line(frame, (0, yCenter), (int(width / 2) - 20, yCenter), (0, 0, 255), 2)
        cv2.line(frame, (int(width / 2) + 20, yCenter), (width, yCenter), (0, 0, 255), 2)
        cv2.line(frame, (xCenter, 0), (xCenter, int(height / 2) - 20), (0, 0, 255), 2)
        cv2.line(frame, (xCenter, int(height / 2) + 20), (xCenter, height), (0, 0, 255), 2)                        
